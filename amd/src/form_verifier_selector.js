 ...
 *
 * NOTE: The code was copied from search identity in core and OLMS programs plugin.
 *
 * @module      assignfeedback_verified/form_verifier_selector
...
export async function transport(selector, query, callback, failure) {
    ...
    // This is a hidden input that holds the assign id.
    const assignId = document.querySelector(selector).closest("form").querySelector("[name=assignid]").value;

    const request = {
        methodname: 'assignfeedback_verified_search_verifier',
        args: {
            query: query,
            assignid: assignId
        }
    ...
    try {
        ...
        if (response.overflow) {
            ...
            response.list.forEach(user => {
                labels.push(renderTemplate('assignfeedback_verified/form_user_selector_suggestion', user));
            });
